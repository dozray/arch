 ...
 * 第二步，你必须配置Spring应用上下文，也许您还记得，你需要配置数据源，
 * 事务管理器和实体管理器工厂的bean。如果您正在使用Spring 3.1和Servlet 3.0，
 * 你可以这样做到，通过实现一个Java配置类，并在您的Web应用程序初始化的时候加载该配置类。
 * 我的应用程序上下文配置类的内容如下下：
 */
@Configuration
@ComponentScan(basePackages = {"net.petrikainulainen.spring.datajpa.controller"})
@EnableWebMvc
//应用上下文配置类使用@ImportResource annotation 来从xml配置文件导入额外的配置. 
//当前Spring Data JPA不支持java配置，因此，唯一的方式是使用XML配置文件
@ImportResource("classpath:applicationContext.xml")
//属性文件的位置，属性文件包含了使用的配置参数的值
